...
class EnsembleBufferBase:
    """
    Temporal ensemble buffer base.
    """
    def __init__(
        self, 
        mode: Literal["new", "old", "avg", "act", "hato"] = "new", 
        **kwargs
    ) -> None:
        """ Initialization. """
        self.mode = mode
    ...
    def get_action(self) -> Union[np.ndarray, torch.Tensor, None]:
...
class EnsembleBuffer:
    ...
    def __init__(
        self, 
        config: EnsembleBufferConfig
    ) -> None:
        """ Initialization. """
        self.ensemble_cfg = config.params_dict
        self.ensemble_dict = {}
        for key, params in self.ensemble_cfg.items():
            self.ensemble_dict[key] = EnsembleBufferBase(**params)
    

    def add_action(
        self, 
        action: Dict[str, Union[np.ndarray, torch.Tensor]], 
        timestep: int
    ) -> None:
        """ Add action dict to buffer. """
        for key in self.ensemble_cfg.keys():
            self.ensemble_dict[key].add_action(action[key], timestep)
    

    def get_action(self) -> Dict[str, Union[np.ndarray, torch.Tensor]]:
        """ Get action dict from buffer. """
        action = {}
        for key in self.ensemble_cfg.keys():
            action[key] = self.ensemble_dict[key].get_action()
        return action
